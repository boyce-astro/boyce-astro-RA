...
def home():
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            if content['min_d_sep'] and content['max_d_sep']:
                if not content['max_d_sep'] > content['min_d_sep']:
                    content['max_d_sep'] = content['min_d_sep'] + 1
            if content['min_nobs'] and content['nobs']:
                if not content['nobs'] > content['min_nobs']:
                    content['nobs'] = content['min_nobs'] + 10
            # Update session['content'] NEED TO ADD TO THIS LATER
            session['content'] = content
            # by min RA
            if content['min_ra']:
            ...
            if content['min_nobs']:
                data = data[data['NOBS'] >= content['min_nobs']]
            # By delta sep
            if content['min_d_sep']:
                data = data[data['delta_sep'] >= content['min_d_sep']]
            if content['max_d_sep']:
                data = data[data['delta_sep'] <= content['max_d_sep']]
